...
const lastNFT = async () => {
    ...
    if(chainId.value == 80001){
        ...
        let result: any = boxId == 0 ? await Web3.tokensOfOwner(cyberClub_Fuji.abi, cyberClub_Fuji.address) : boxId == 1 ? await Web3.tokensOfOwner(game_Fuji.abi, game_Fuji.address) : await Web3.tokensOfOwner(Cyborg_Fuji.abi, Cyborg_Fuji.address);
        nftID.value = result[result.length-1]
    }
}
